        ...
        class TaskManagerNode(Node):
            ...
            def timer_callback(self):
                self.get_logger().info(
                    f"waiting_for_pose={self.waiting_for_pose}, current_target_index={self.current_target_index}")
                self.get_logger().info(f"has_received_color={self.has_received_color}, last_color={self.last_color}")
                self.get_logger().info(
                    f"has_received_pose={self.has_received_pose}, last_pose={'set' if self.last_pose else 'None'}")
                ...
                if not self.waiting_for_pose:
                    self.get_logger().info("Venter på at roboten skal være klar (waiting_for_pose=False)")
                    return
                ...
                self.get_logger().info(f"Forventer farge: {current_target_color}, mottatt farge: {self.last_color}")
            ...
            def task_done_callback(self, request, response):
                self.get_logger().info(
                    "Robot rapporterte at kube er håndtert. Kaller go_to_home og gjør klar for neste.")
